(function() {
	//document.write('<script type="text/javascript" src="http://app.cloopen.com/im50/MD5.min.js"><\/script>');
	//document.write('<script type="text/javascript" src="http://app.cloopen.com/im50/base64.min.js"><\/script>');
	//document.write('<script type="text/javascript" src="http://app.cloopen.com/im50/jquery-1.11.3.min.js"><\/script>');
	var YTX_CONFIG = {
		_server_ip: ["d3M6Ly8xMjMuNTcuMjMwLjE1ODo4MTgwL3dz"],
		_file_server_url: "d3M6Ly8xMjMuNTcuMjMwLjE1ODo4MTgwL3dz",
		_lvs_servers: ["aHR0cDovLzEyMy41Ni4xMzUuODE6ODg4OA=="],
		_file_sig_url: "aHR0cDovLzEyMy41Ny4yMzAuMTU4Ojg4ODYvc2lnL2ZpbGU=",
		_socket: null,
		_appid: "",
		_version: "5.2.0r",
		_ClientNo: 0,
		...
		_log: function(lev, content) {
			...
			if (lev == YTX_CONFIG._logLev._DEBUG) {
				...
				if (lev == YTX_CONFIG._logLev._INFO) {
					//console.info(content)
				} else {
					if (lev == YTX_CONFIG._logLev._WARN) {
						console.warn(content)
					} else {
						if (lev == YTX_CONFIG._logLev._ERROR) {
		...
		_sendMsg: function(content) {
			...
			if (!YTX_CONFIG._requestTime) {
				...
				if ((curTime - YTX_CONFIG._requestTime) > 60 * 1000) {
					YTX_CONFIG._requestTime = curTime;
					YTX_CONFIG._requestCounter = 0
				}
			}
		...
		_noticeApp: function(obj) {
			...
			if (obj.msgType == YTX_CONFIG._prototype._msg_CMD) {
				...
				if (obj.msgType == YTX_CONFIG._prototype._msg_DEL) {
		...
		_processSyncMsg: function(type, endVersion, requestType) {
			...
			if (!!endVersion && endVersion < YTX_CONFIG._maxMsgVersion) {
				var type = (!!requestType) ? requestType : 0;
		...
		_onResponse: function(obj) {
			...
			if (type == YTX_CONFIG._prototype._callRoute) {
				...
				if (type == YTX_CONFIG._prototype._pushMsg) {
					...
					if (YTX_CONFIG._msgVersion + 1 == pushMsg.version) {
						...
						if (pushMsg.version > YTX_CONFIG._msgVersion) {
					...
					if (type == YTX_CONFIG._prototype._pushMsgNotify) {
						...
						if (type == YTX_CONFIG._prototype._syncMsg) {
							...
							for (var i in resps) {
								...
								if (YTX_CONFIG._msgVersion >= resp.version) {
									...
									if (YTX_CONFIG._msgVersion + 1 == resp.version) {
							...
							if (type == YTX_CONFIG._prototype._mcmEventData._prototype) {
								...
								if (type == YTX_CONFIG._prototype._kickOff) {
			...
			if (type == YTX_CONFIG._prototype._login) {
				...
				if (type == YTX_CONFIG._prototype._logout) {
					YTX_CONFIG._logout();
					...
					if (type == YTX_CONFIG._prototype._sendMsg) {
						...
						if (type == YTX_CONFIG._prototype._getMyInfo || type == YTX_CONFIG._prototype._setMyInfo) {
							callback(YTX_CONFIG._protobuf._parseGetMyInfo(obj))
						} else {
							if (type == YTX_CONFIG._prototype._createGroup) {
								callback(YTX_CONFIG._protobuf._parseCreateGroupResp(obj))
							} else {
								if (type == YTX_CONFIG._prototype._getOwnGroups) {
									callback(YTX_CONFIG._protobuf._parseGetGroupListResp(obj))
								} else {
									if (type == YTX_CONFIG._prototype._queryGroupMembers) {
										callback(YTX_CONFIG._protobuf._parseGetGroupMemberListResp(obj))
									} else {
										if (type == YTX_CONFIG._prototype._getGroupDetail) {
											callback(YTX_CONFIG._protobuf._parseGetGroupDetailResp(obj))
										} else {
											if (type == YTX_CONFIG._prototype._searchGroups) {
												callback(YTX_CONFIG._protobuf._parseSearchGroupsResp(obj))
											} else {
												if (type == YTX_CONFIG._prototype._queryGroupMemberCard) {
													callback(YTX_CONFIG._protobuf._parseQueryGroupMemberCard(obj))
												} else {
													if (type == YTX_CONFIG._prototype._getUserState) {
		...
		_onHttpResonse: function(obj) {
			...
			if (type == YTX_CONFIG._httpType._historyMessage) {
				...
				if (type == YTX_CONFIG._httpType._recentContact) {
					var data = obj;
		...
		_processVoip: function(callEventData) {
			...
			if (callEventData.getCallEvent() == 1) {
				if (!YTX_CONFIG._voipCallData._callEventCallId) {
					...
					if (sdpStr.indexOf("m=audio") > -1) {
						if (sdpStr.indexOf("m=video") > -1) {
							type = 1;
							YTX_CONFIG._voipCallData._isVideo = true
						}
					}
					resp.callType = type;
					resp.state = 6;
					...
					if (callEventData.getCallId() == YTX_CONFIG._voipCallData._callEventCallId) {
						return
					} else {
						if (callEventData.getCallId() != YTX_CONFIG._voipCallData._callEventCallId) {
							callEventData.setCallEvent(10);
				...
				if (callEventData.getCallEvent() == 2 || callEventData.getCallEvent() == 3) {
					var resp = {};
					resp.callId = callEventData.getCallId();
					resp.caller = callEventData.getCaller();
					resp.called = callEventData.getCalled();
					...
					resp.state = 1;
					var type = 0;
					if (YTX_CONFIG._voipCallData._isVideo) {
						type = 1
					}
					resp.callType = type;
					resp.code = 200;
					...
					if (callEventData.getCallEvent() == 4) {
						...
						if (callEventData.getCallEvent() == 6) {} else {
							if (callEventData.getCallEvent() == 7 || callEventData.getCallEvent() == 8) {
								var resp = {};
								resp.callId = callEventData.getCallId();
								resp.caller = callEventData.getCaller();
								resp.called = callEventData.getCalled();
								...
								if (callEventData.getCallEvent() == 10) {
									var resp = {};
									resp.callId = callEventData.getCallId();
									resp.caller = callEventData.getCaller();
									resp.called = callEventData.getCalled();
									...
									resp.code = 200;
									resp.userdata = callEventData.getUserData();
									if (callEventData.getReason() == 486) {
										resp.reason = 2
									}
									var type = 0;
									if (YTX_CONFIG._voipCallData._isVideo) {
									...
									if (callEventData.getCallEvent() == 11 || callEventData.getCallEvent() == 12) {
										YTX_CONFIG._voipReply200(callEventData)
									} else {
										if (callEventData.getCallEvent() == 15) {
		...
		_onResponseErr: function(obj) {
			...
			if (obj["1"] == YTX_CONFIG._prototype._getMyInfo && obj["6"] == 520015) {
				...
				if ((obj["6"] >= 520001 && obj["6"] <= 520010) || (obj["6"] >= 520018 && obj["6"] <= 520019) || (obj["6"] >= 520021 && obj["6"] <= 520021) || (obj["6"] >= 520023 && obj["6"] <= 520023) || obj["6"] == 529998) {
					var onError = request.onError;
					onError(YTX_CONFIG._protobuf._parseCodeResp(obj));
					return
				} else {
					if ((obj["6"] >= 520000 && obj["6"] <= 529999) || (obj["6"] >= 550000 && obj["6"] <= 559999) || obj["6"] == 219000) {
		...
		_protobuf: {
			...
			_buildSendTextMsg: function(content, receiver, msgId, msgDomain, callback, onError, orignMsgId) {
				...
				if (!content || !receiver) {
					...
					resp.msg = "param content or receiver is empty";
					onError(resp);
					return
				}
				...
				sendJsonStr["1"] = YTX_CONFIG._msgType._TEXT;
				sendJsonStr["2"] = msgId + "";
			...
			_buildMCM_UserEvt_StartAsk: function(osUnityAccount, agentId, userData, callback, onError) {
			...
			_buildMCM_AgentEvt_StartSerWithUser: function(userAccount, MCMDataBuilder, userData, callback, onError) {
				if (!YTX_CONFIG._checkOnline(onError)) {
					return null
				}
				if (!userAccount) {
				...
				jsonStr["4"] = userAccount;
				jsonStr["10"] = YTX_CONFIG._mcm_agentId;
			...
			_buildMCM_AgentEvt_StopSerWithUser: function(userAccount, userData, callback, onError) {
				if (!YTX_CONFIG._checkOnline(onError)) {
					return null
				}
				if (!userAccount) {
			...
			_buildMCM_AgentEvt_KFOnWork: function(serverCap, MCMAgentInfoBuilder, callback, onError) {
				...
				if (!MCMAgentInfoBuilder.getAgentId()) {
					var resp = {};
					resp.code = YTX_CONFIG._errcode._NO_REQUIRED_PARAM;
				...
				if ((!!MCMAgentInfoBuilder.getImState()) && isNaN(MCMAgentInfoBuilder.getImState())) {
					...
					if ((!!MCMAgentInfoBuilder.getTelState()) && isNaN(MCMAgentInfoBuilder.getTelState())) {
						errorStr = "TelState"
					} else {
						if ((!!MCMAgentInfoBuilder.getDelayCall()) && isNaN(MCMAgentInfoBuilder.getDelayCall())) {
							errorStr = "DelayCall"
						} else {
							if ((!!MCMAgentInfoBuilder.getAnswerTimeout()) && isNaN(MCMAgentInfoBuilder.getAnswerTimeout())) {
								errorStr = "AnswerTimeout"
							} else {
								if ((!!MCMAgentInfoBuilder.getMaxImUser()) && isNaN(MCMAgentInfoBuilder.getMaxImUser())) {
									errorStr = "MaxImUser"
								} else {
									if ((!!MCMAgentInfoBuilder.getAgentServerMode()) && isNaN(MCMAgentInfoBuilder.getAgentServerMode())) {
										errorStr = "AgentServerMode"
									} else {
										if ((!!serverCap) && isNaN(serverCap)) {
											errorStr = "serverCap"
										}
									}
								}
				...
				YTX_CONFIG._mcm_agentId = MCMAgentInfoBuilder.getAgentId();
				var jsonStr = new Object();
				...
				jsonStr13["1"] = YTX_CONFIG._mcm_agentId;
			...
			_buildMCM_AgentEvt_KFOffWork: function(callback, onError) {
				if (!YTX_CONFIG._checkOnline(onError)) {
					return null
				}
				if (!YTX_CONFIG._mcm_agentId) {
					var resp = {};
					resp.code = YTX_CONFIG._errcode._NO_REQUIRED_PARAM;
					resp.msg = "the MCM agent don't on work!";
					onError(resp);
					return
				}
				var jsonStr = new Object();
			...
			_buildMCM_AgentEvt_Ready: function(callback, onError) {
				if (!YTX_CONFIG._checkOnline(onError)) {
					return null
				}
				if (!YTX_CONFIG._mcm_agentId) {
			...
			_buildMCM_AgentEvt_NotReady: function(callback, onError) {
				if (!YTX_CONFIG._checkOnline(onError)) {
					return null
				}
				if (!YTX_CONFIG._mcm_agentId) {
			...
			_buildMCM_AgentEvt_RejectUser: function(userAccount, ccpCustomData, callback, onError) {
				if (!YTX_CONFIG._checkOnline(onError)) {
					return null
				}
				if (!YTX_CONFIG._mcm_agentId) {
			...
			_buildMCM_AgentEvt_TransKF: function(userAccount, osUnityAccount, transAgentId, callback, onError) {
				if (!YTX_CONFIG._checkOnline(onError)) {
					return null
				}
				if (!YTX_CONFIG._mcm_agentId) {
			...
			_buildMCM_AgentEvt_MonitorAgent: function(userAccount, callback, onError) {
				if (!YTX_CONFIG._checkOnline(onError)) {
					return null
				}
				if (!userAccount) {
			...
			_buildMCM_AgentEvt_CancelMonitorAgent: function(userAccount, callback, onError) {
				if (!YTX_CONFIG._checkOnline(onError)) {
					return null
				}
				if (!userAccount) {
			...
			_buildMCM_AgentEvt_QueryAgentInfo: function(callback, onError) {
				if (!YTX_CONFIG._checkOnline(onError)) {
					return null
				}
				if (!YTX_CONFIG._mcm_agentId) {
			...
			_buildMCM_AgentEvt_ReservedForUser: function(keyType, reservedKey, callback, onError) {
				if (!YTX_CONFIG._checkOnline(onError)) {
					return null
				}
				if (!YTX_CONFIG._mcm_agentId) {
			...
			_buildMCM_AgentEvt_CancelReserved: function(keyType, reservedKey, callback, onError) {
				if (!YTX_CONFIG._checkOnline(onError)) {
					return null
				}
				if (!YTX_CONFIG._mcm_agentId) {
			...
			_buildMCM_AgentEvt_SerWithTheUser: function(osUnityAccount, userAccount, chanType, callback, onError) {
				if (!YTX_CONFIG._checkOnline(onError)) {
					return null
				}
				if (!YTX_CONFIG._mcm_agentId) {
			...
			_buildCallEvent: function(CallEventBuilder, callback, onError) {
				...
				if (!CallEventBuilder || !CallEventBuilder.getCalled() || !CallEventBuilder.getCaller() || !CallEventBuilder.getCalled()) {
					var resp = {};
					resp.code = YTX_CONFIG._errcode._NO_REQUIRED_PARAM;
			...
			buildGetRecentContactList: function(GetRecentContactListBuilder, callback, onError) {
				if (!YTX_CONFIG._checkOnline(onError, null)) {
					return null
				}
				var sig = YTX_CONFIG._fileSig;
			...
			_parseMCM_Msg: function(data) {
				...
				if (msg.mcmEvent == YTX_CONFIG._prototype._mcmEventData._mcmEventDef._AgentEvt_SendMCM) {
					...
					if (msg.mcmEvent == YTX_CONFIG._prototype._mcmEventData._mcmEventDef._UserEvt_SendMSG) {
			...
			_parseGetHistoryMsg: function(obj) {
				...
				for (var i in data) {
					...
					if (orign.msgType == YTX_CONFIG._prototype._msg_CMD) {
						...
						var groupTarget = (msg.target == 2) ? "Èº×é" : "ÌÖÂÛ×é";
						if (1 == auditType) {
							you_msgContent = "[" + people + "]ÉêÇë¼ÓÈë" + groupTarget + "[" + groupName + "]"
						} else {
							if (2 == auditType) {
								you_msgContent = "[" + groupName + "]¹ÜÀíÔ±ÑûÇëÄú¼ÓÈë" + groupTarget
							} else {
								if (3 == auditType) {
									you_msgContent = "[" + people + "]Ö±½Ó¼ÓÈëÈº×é[" + groupName + "]"
								} else {
									if (4 == auditType) {
										you_msgContent = "¹ÜÀíÔ±½âÉ¢ÁËÈº×é[" + groupName + "]"
									} else {
										if (5 == auditType) {
											you_msgContent = "[" + people + "]ÍË³öÁË" + groupTarget + "[" + groupName + "]"
										} else {
											if (6 == auditType) {
												you_msgContent = "[" + groupName + "]¹ÜÀíÔ±½«[" + people + "]Ìß³ö" + groupTarget
											} else {
												if (7 == auditType) {
													you_msgContent = "¹ÜÀíÔ±Í¬Òâ[" + people + "]¼ÓÈëÈº×é[" + groupName + "]µÄÉêÇë"
												} else {
													if (8 == auditType) {
														if (2 != obj.confirm) {
															you_msgContent = "[" + people + "]¾Ü¾øÁËÈº×é[" + groupName + "]µÄÑûÇë"
														} else {
															you_msgContent = "[" + people + "]Í¬ÒâÁË¹ÜÀíÔ±µÄÑûÇë£¬¼ÓÈëÈº×é[" + groupName + "]"
														}
													} else {
														if (10 == auditType) {
															you_msgContent = "¹ÜÀíÔ±ÐÞ¸Ä" + groupTarget + "[" + groupName + "]ÐÅÏ¢"
														} else {
															if (11 == auditType) {
																you_msgContent = "ÓÃ»§[" + people + "]ÐÞ¸ÄÈº×é³ÉÔ±ÃûÆ¬"
															} else {
																if (12 == auditType) {
																	you_msgContent = "ÓÃ»§[" + people + "]³ÉÎª" + groupTarget + "[" + groupName + "]¹ÜÀíÔ±"
																} else {
																	you_msgContent = "Î´Öªtype[" + auditType + "]"
																}
															}
														}
													}
			...
			_parseGetGroupListResp: function(obj) {
				var data = obj["2"];
				var resp = new Array();
				if (!data) {
					return resp
			...
			_parseGetGroupMemberListResp: function(obj) {
				var data = obj["2"];
				var resp = new Array();
				if (!data) {
					return resp
			...
			_parseCallEventData: function(obj) {
				...
				var resp = new YTX_CONFIG._protobuf._CallEventData(data["1"], data["2"], data["3"], data["5"], data["7"], data["13"], data["17"]);
				return resp
			},
		...
		_sendFile: function(fileInfo, content, receiver, msgType, msgId, msgDomain, type, callback, onError, progress, fileName, orignMsgId) {
			...
			if (isNaN(msgType)) {
				...
				resp.msgId = orignMsgId;
				resp.msgClientNo = msgId;
				resp.msg = "param msgType is not a number.";
				onError(resp);
				return
			}
			var sendStatus = 1;
				...
				reader.onload = function loaded(evt) {
					...
					while (start < total) {
						...
						if (binaryString.slice) {
							...
							if (binaryString.mozSlice) {
								part = binaryString.mozSlice(start, end)
							} else {
								if (binaryString.webkitSlice) {
		...
		_releaseVoip: function() {
			...
			YTX_CONFIG._voipCallData._isVideo = false
		},
			...
			for (var i in arr) {
				...
				for (var j in bodyArr) {
					if (bodyArr[j].indexOf("rtpmap") > 0) {
						...
						if (bodyArr[j].indexOf("PCMA") > 0 || bodyArr[j].indexOf("ISAC") > 0 || bodyArr[j].indexOf("G722") > 0 || bodyArr[j].indexOf("opus") > 0 || bodyArr[j].indexOf("H264") > 0 || bodyArr[j].indexOf("rtx") > 0) {
							delCode = code;
							continue
						}
						newHead += " " + code;
						...
						if (bodyArr[j].indexOf("fmtp") > 0) {
							...
							if (bodyArr[j].indexOf("candidate") > 0) {
								...
								if (bodyArr[j].indexOf("c=") == 0) {
									newBody += tabStr + "c=IN IP4 " + localIp
								} else {
									newBody += tabStr + bodyArr[j]
								}
							}
		...
		_sendVoip: function(CallEventBuilder, callback, onError) {
			...
			if (getUserMedia) {
				...
				if (!YTX_CONFIG._voipCallData._voipOtherView) {
					if (CallEventBuilder.getIsVoipCall() == 0) {
				...
				var isVideo = null;
				if (!CallEventBuilder.getIsVoipCall()) {
					isVideo = YTX_CONFIG._voipCallData._isVideo
				} else {
					if (CallEventBuilder.getIsVoipCall() == 1) {
						YTX_CONFIG._voipCallData._isVideo = true;
						isVideo = true
					} else {
						isVideo = false
				...
				getUserMedia.call(navigator, {
					...
					if (isCaller) {
						...
						if ("FF" == mb) {
							if (inviteSdp.indexOf("a=rtpmap:0 PCMU/8000\r\n") < 0) {
								var maLen = inviteSdp.indexOf("m=audio");
								var str1 = inviteSdp.substring(0, maLen);
								var str2 = inviteSdp.substring(maLen, inviteSdp.length - 1);
	...
	window.RL_YTX = window.RL_YTX || {
		...
		sendMsg: function(MsgBuilder, callback, onError, progress) {
			...
			if (MsgBuilder.getType() != 1 && !!MsgBuilder.getFile()) {
				...
				var sendStr = YTX_CONFIG._protobuf._buildSendTextMsg(MsgBuilder.getText(), MsgBuilder.getReceiver(), msgToken, MsgBuilder.getDomain(), callback, onError, MsgBuilder.getId());
				if (!!sendStr) {
					YTX_CONFIG._sendMsg(sendStr)
				}
		...
		mcmUserStartAsk: function(osUnityAccount, agentId, callback, onError) {
			var sendStr = YTX_CONFIG._protobuf._buildMCM_UserEvt_StartAsk(osUnityAccount, agentId, null, callback, onError);
			if (!!sendStr) {
				YTX_CONFIG._sendMsg(sendStr)
			}
		},
		...
		mcmUserSendMsg: function(MCMMessageBuilder, callback, onError, progress) {
			var msgId = null;
			if (MCMMessageBuilder.getType() != 1 && !!MCMMessageBuilder.getFile()) {
		...
		mcmAgentStartSerWithUser: function(userAccount, MCMDataBuilder, callback, onError) {
			var sendStr = YTX_CONFIG._protobuf._buildMCM_AgentEvt_StartSerWithUser(userAccount, MCMDataBuilder, null, callback, onError);
			if (!!sendStr) {
				YTX_CONFIG._sendMsg(sendStr)
			}
		},
		mcmAgentStopSerWithUser: function(userAccount, callback, onError) {
			var sendStr = YTX_CONFIG._protobuf._buildMCM_AgentEvt_StopSerWithUser(userAccount, null, callback, onError);
			if (!!sendStr) {
				YTX_CONFIG._sendMsg(sendStr)
			}
		},
		...
		mcmAgentKFOffWork: function(callback, onError) {
			var sendStr = YTX_CONFIG._protobuf._buildMCM_AgentEvt_KFOffWork(callback, onError);
			if (!!sendStr) {
				YTX_CONFIG._sendMsg(sendStr)
			}
		},
		mcmAgentReady: function(callback, onError) {
			var sendStr = YTX_CONFIG._protobuf._buildMCM_AgentEvt_Ready(callback, onError);
			if (!!sendStr) {
				YTX_CONFIG._sendMsg(sendStr)
			}
		},
		mcmAgentNotReady: function(callback, onError) {
			var sendStr = YTX_CONFIG._protobuf._buildMCM_AgentEvt_NotReady(callback, onError);
			if (!!sendStr) {
				YTX_CONFIG._sendMsg(sendStr)
			}
		},
		mcmAgentRejectUser: function(userAccount, ccpCustomData, callback, onError) {
			var sendStr = YTX_CONFIG._protobuf._buildMCM_AgentEvt_RejectUser(userAccount, ccpCustomData, callback, onError);
			if (!!sendStr) {
				YTX_CONFIG._sendMsg(sendStr)
			}
		},
		mcmAgentTransKF: function(userAccount, osUnityAccount, transAgentId, callback, onError) {
			var sendStr = YTX_CONFIG._protobuf._buildMCM_AgentEvt_TransKF(userAccount, osUnityAccount, transAgentId, callback, onError);
			if (!!sendStr) {
				YTX_CONFIG._sendMsg(sendStr)
			}
		},
		...
		mcmAgentMonitorAgent: function(userAccount, callback, onError) {
			var sendStr = YTX_CONFIG._protobuf._buildMCM_AgentEvt_MonitorAgent(userAccount, callback, onError);
			if (!!sendStr) {
				YTX_CONFIG._sendMsg(sendStr)
			}
		},
		mcmAgentCancelMonitorAgent: function(userAccount, callback, onError) {
			var sendStr = YTX_CONFIG._protobuf._buildMCM_AgentEvt_CancelMonitorAgent(userAccount, callback, onError);
			if (!!sendStr) {
				YTX_CONFIG._sendMsg(sendStr)
			}
		},
		...
		mcmAgentQueryAgentInfo: function(callback, onError) {
			var sendStr = YTX_CONFIG._protobuf._buildMCM_AgentEvt_QueryAgentInfo(callback, onError);
			if (!!sendStr) {
				YTX_CONFIG._sendMsg(sendStr)
			}
		},
		...
		mcmAgentReservedForUser: function(keyType, reservedKey, callback, onError) {
			var sendStr = YTX_CONFIG._protobuf._buildMCM_AgentEvt_ReservedForUser(keyType, reservedKey, callback, onError);
			if (!!sendStr) {
				YTX_CONFIG._sendMsg(sendStr)
			}
		},
		mcmAgentCancelReserved: function(keyType, reservedKey, callback, onError) {
			var sendStr = YTX_CONFIG._protobuf._buildMCM_AgentEvt_CancelReserved(keyType, reservedKey, callback, onError);
			if (!!sendStr) {
				YTX_CONFIG._sendMsg(sendStr)
			}
		},
		mcmAgentSerWithTheUser: function(osUnityAccount, userAccount, chanType, callback, onError) {
			var sendStr = YTX_CONFIG._protobuf._buildMCM_AgentEvt_SerWithTheUser(osUnityAccount, userAccount, chanType, callback, onError);
			if (!!sendStr) {
				YTX_CONFIG._sendMsg(sendStr)
			}
		},
		...
		setCallView: function(view, localView) {
		...
		makeCall: function(MakeCallBuilder, callback, onError) {
			...
			var CallEventBuilder = new YTX_CONFIG._protobuf._CallEventData(1, callId, MakeCallBuilder.getCallType(), MakeCallBuilder.getCalled(), YTX_CONFIG._userName);
		...
		getRecentContactList: function(GetRecentContactListBuilder, callback, onError) {
			var sendStr = YTX_CONFIG._protobuf.buildGetRecentContactList(GetRecentContactListBuilder, callback, onError);
			if (!!sendStr) {
				YTX_CONFIG._sendMsg(sendStr)
			}
		},
		...
		MsgBuilder: function(id, content, file, type, receiver, domain, fileName) {
			this._id = id;
			this._text = content;
			this._file = file;
		...
		GetUserStateBuilder: function(useracc) {
				...
				function onErr(error) {
					...
					switch (error.code || error.name) {
						...
						case "PermissionDeniedError":
							YTX_CONFIG._log(YTX_CONFIG._logLev._INFO, "ÓÃ»§¾Ü¾øÌá¹©ÐÅÏ¢¡£");
							resp.msg = "user refuesed";
							break;
						case "NOT_SUPPORTED_ERROR":
						case "NotSupportedError":
							YTX_CONFIG._log(YTX_CONFIG._logLev._INFO, "ä¯ÀÀÆ÷²»Ö§³ÖÓ²¼þÉè±¸¡£");
							resp.msg = "brower not support";
							break;
						case "MANDATORY_UNSATISFIED_ERROR":
						case "MandatoryUnsatisfiedError":
							YTX_CONFIG._log(YTX_CONFIG._logLev._INFO, "ÎÞ·¨·¢ÏÖÖ¸¶¨µÄÓ²¼þÉè±¸¡£");
							resp.msg = "can not find device";
							break;
						default:
							YTX_CONFIG._log(YTX_CONFIG._logLev._INFO, "ÎÞ·¨´ò¿ªÒôÊÓÆµ¡£Òì³£ÐÅÏ¢:" + (error.code || error.name));
				...
				function onErr(error) {
					...
					switch (error.code || error.name) {
						...
						case "MandatoryUnsatisfiedError":
							...
							break;
						default:
							YTX_CONFIG._log(YTX_CONFIG._logLev._INFO, "ÎÞ·¨´ò¿ªÂó¿Ë·ç¡£Òì³£ÐÅÏ¢:" + (error.code || error.name));
							resp.msg = "can not open resource";
							break
					}
					resp.code = YTX_CONFIG._errcode._VOIP_NO_MEDIA;
			...
			encodeWAV: function() {
				...
				if (sampleBits === 8) {
					...
					for (var i = 0; i < bytes.length; i++, offset += 2) {
						var s = Math.max(-1, Math.min(1, bytes[i]));
						data.setInt16(offset, s < 0 ? s * 32768 : s * 32767, true)
					}
				}
	...
	HZRecorder.get = function(callback, config) {
		if (callback) {
			...
			if (userMedia) {
				if (HZRecorder.recorderIndex == 0) {
					...
					userMedia({
						...
						switch (error.code || error.name) {
							case "PERMISSION_DENIED":
							case "PermissionDeniedError":
								HZRecorder.throwError("ÓÃ»§¾Ü¾øÌá¹©ÐÅÏ¢¡£");
								break;
							case "NOT_SUPPORTED_ERROR":
							case "NotSupportedError":
								HZRecorder.throwError("ä¯ÀÀÆ÷²»Ö§³ÖÓ²¼þÉè±¸¡£");
								break;
							case "MANDATORY_UNSATISFIED_ERROR":
							case "MandatoryUnsatisfiedError":
								HZRecorder.throwError("ÎÞ·¨·¢ÏÖÖ¸¶¨µÄÓ²¼þÉè±¸¡£");
								break;
							default:
								HZRecorder.throwError("ÎÞ·¨´ò¿ªÂó¿Ë·ç¡£Òì³£ÐÅÏ¢:" + (error.code || error.name));
								break
						}
						HZRecorder.recorderIndex = 0
					})
				...
				HZRecorder.throwError("µ±Ç°ä¯ÀÀÆ÷²»Ö§³ÖÂ¼Òô¹¦ÄÜ¡£");
				return
			}
		}
	};
